...
theorem from_montgomery_loop_spec (m : Scalar52) :
    ...
    rw [show Finset.range 9 = Finset.range 5 ∪ Finset.Ico 5 9 by ext x; simp; omega]
    rw [Finset.sum_union (by decide)]

    have h_zero : ∀ i, 5 ≤ i → i < 9 → a_5[i]! = 0#u128 := by
      intro i _ _; interval_cases i <;> simp [ha_5, ha_4, ha_3, ha_2, ha_1, Array.repeat]

    have : ∑ x ∈ Finset.Ico 5 9, 2 ^ (52 * x) * (a_5[x]! : ℕ) = 0 := by
      refine Finset.sum_eq_zero fun i hi => ?_; simp [Finset.mem_Ico] at hi
      rw [h_zero i hi.1 hi.2]; simp

    rw [this, add_zero]; refine Finset.sum_congr rfl fun i hi => ?_; simp [Finset.mem_range] at hi
    interval_cases i <;> simp [ha_5, ha_4, ha_3, ha_2, ha_1, Array.repeat,
      h2_5, h2_4, h2_3, h2_2, h2_1, h1_5, h1_4, h1_3, h1_2, h1_1]
...
theorem from_montgomery_loop_spec (m : Scalar52) :
    ...
    have h_a5_zero : ∀ i : ℕ, 5 ≤ i → i < 9 → a_5[i]! = (0#u128) := by
      intro i _ _; interval_cases i <;> simp [ha_5, ha_4, ha_3, ha_2, ha_1, Array.repeat]
    have h_a5_eq_m : ∀ i : ℕ, i < 5 → (a_5[i]! : ℕ) = (m[i]! : ℕ) := by
      intro i _; interval_cases i <;> simp [ha_5, ha_4, ha_3, ha_2, ha_1, Array.repeat,
        h2_5, h2_4, h2_3, h2_2, h2_1, h1_5, h1_4, h1_3, h1_2, h1_1]
    have : (∑ x ∈ Finset.Ico 5 9, 2 ^ (52 * x) * (a_5[x]! : ℕ)) = 0 := by
      apply Finset.sum_eq_zero; intro i hi; simp [Finset.mem_Ico] at hi; rw [h_a5_zero i hi.1 hi.2]; simp
    rw [this, add_zero]; apply Finset.sum_congr rfl; intro i hi; simp [Finset.mem_range] at hi; rw [h_a5_eq_m i hi]
