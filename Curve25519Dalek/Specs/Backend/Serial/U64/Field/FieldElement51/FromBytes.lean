...
theorem load8_at_spec_bitwise (input : Slice U8) (i : Usize)
  ...
  · rw [hc]
    ...
    repeat rw [Nat.mod_eq_of_lt (U8_shiftLeft_lt (by grind) _)]
    repeat rw [Nat.testBit_shiftLeft]
    rw [show j % 8 = j - 8 by grind]
    repeat rw [byte_testBit_of_ge _ (by grind)]
    simp only [ge_iff_le]
    rw [show decide (16 ≤ j) = false by rw [decide_eq_false_iff_not]; grind]
    rw [show decide (24 ≤ j) = false by rw [decide_eq_false_iff_not]; grind]
    rw [show decide (32 ≤ j) = false by rw [decide_eq_false_iff_not]; grind]
    rw [show decide (40 ≤ j) = false by rw [decide_eq_false_iff_not]; grind]
    rw [show decide (48 ≤ j) = false by rw [decide_eq_false_iff_not]; grind]
    rw [show decide (56 ≤ j) = false by rw [decide_eq_false_iff_not]; grind]
    all_goals grind
  · rw [hc]
    have : j < 24 := by grind
    have : 16 ≤ j := by grind
