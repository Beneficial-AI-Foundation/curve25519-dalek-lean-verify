...
namespace curve25519_dalek.backend.serial.u64.field.FieldElement51.v2
...
-- Re-prove step specs in our namespace so @[step] is scoped here
@[step]
...
/-- Squaring ModEq: given carry chain conservation and the algebraic identity,
    Field51_as_Nat of the reduced array is congruent to (Field51_as_Nat a)^2 mod p. -/
private lemma field51_sq_modeq
    (a' a'' : Array U64 5#usize) (carry : U64)
    (c0 c1 c2 c3 c4 : ℕ) (c1' c2' c3' c4' : ℕ)
    -- Array values after carry propagation
    (ha'_0 : a'[0]!.val = c0 % 2 ^ 51)
    (ha'_1 : a'[1]!.val = c1' % 2 ^ 51)
    (ha'_2 : a'[2]!.val = c2' % 2 ^ 51)
    (ha'_3 : a'[3]!.val = c3' % 2 ^ 51)
    (ha'_4 : a'[4]!.val = c4' % 2 ^ 51)
    -- Array values after final reduction
    (ha''_0 : a''[0]!.val = (a'[0]!.val + 19 * carry.val) % 2 ^ 51)
    (ha''_1 : a''[1]!.val = a'[1]!.val + (a'[0]!.val + 19 * carry.val) / 2 ^ 51)
    (ha''_2 : a''[2]!.val = a'[2]!.val)
    (ha''_3 : a''[3]!.val = a'[3]!.val)
    (ha''_4 : a''[4]!.val = a'[4]!.val)
    -- Carry chain equalities
    (hc1'_eq : c1' = c1 + c0 / 2 ^ 51) (hc2'_eq : c2' = c2 + c1' / 2 ^ 51)
    (hc3'_eq : c3' = c3 + c2' / 2 ^ 51) (hc4'_eq : c4' = c4 + c3' / 2 ^ 51)
    (hcarry_val : carry.val = c4' / 2 ^ 51)
    -- The original array
    {a : Array U64 5#usize}
    -- Algebraic identity
  ...
  have hf_a'' : Field51_as_Nat a'' =
    ...
    omega
  have h_chain : c0 + 2^51 * c1 + 2^102 * c2 + 2^153 * c3 + 2^204 * c4 =
      a'[0]!.val + 2^51 * a'[1]!.val + 2^102 * a'[2]!.val + 2^153 * a'[3]!.val + 2^204 * a'[4]!.val +
      2^255 * carry.val :=
    carry_chain_eq c0 c1 c2 c3 c4
      a'[0]!.val a'[1]!.val a'[2]!.val a'[3]!.val a'[4]!.val carry.val c1' c2' c3' c4'
      ha'_0 ha'_1 ha'_2 ha'_3 ha'_4 hc1'_eq hc2'_eq hc3'_eq hc4'_eq hcarry_val
  have h_key : Field51_as_Nat a'' + carry.val * p =
      c0 + 2^51 * c1 + 2^102 * c2 + 2^153 * c3 + 2^204 * c4 := by
...
theorem pow2k_loop_spec (k : U32) (a : Array U64 5#usize)
  ...
  case isTrue hlt =>
    ...
    have ha6_0 : a6[0]!.val = a'[0]!.val + 19 * carry.val := by
      simp only [*, Array.set_of_eq _ _ 0 (by grind)]; grind
    have ha6_1 : a6[1]!.val = a'[1]!.val := by
      simp only [*, Array.set_of_ne_getElem! _ _ 1 0 (by grind) (by grind) (by omega)]
    ...
    have ha''_0 : a''[0]!.val = (a'[0]!.val + 19 * carry.val) % 2 ^ 51 := by
      simp only [a''_post, Array.set_of_eq _ _ 0 (by grind)]
      simp only [i61_post_1, UScalar.val_and, h60_val]
      simp only [a7_post, Array.set_of_ne_getElem! _ _ 0 1 (by grind) (by grind) (by omega)]
      simp only [ha6_0]
      simp_all only [Nat.and_two_pow_sub_one_eq_mod]
    have ha''_1 : a''[1]!.val = a'[1]!.val + (a'[0]!.val + 19 * carry.val) / 2 ^ 51 := by
      simp only [a''_post, Array.set_of_ne_getElem! _ _ 1 0 (by grind) (by grind) (by omega)]
      simp only [a7_post, Array.set_of_eq _ _ 1 (by grind)]
      simp only [i59_post, h58_val, ha6_1, h_i57]
    have ha''_2 : a''[2]!.val = a'[2]!.val := by
      simp only [a''_post, Array.set_of_ne_getElem! _ _ 2 0 (by grind) (by grind) (by omega)]
      simp only [a7_post, Array.set_of_ne_getElem! _ _ 2 1 (by grind) (by grind) (by omega)]
      simp only [a6_post, Array.set_of_ne_getElem! _ _ 2 0 (by grind) (by grind) (by omega)]
    have ha''_3 : a''[3]!.val = a'[3]!.val := by
      simp only [a''_post, Array.set_of_ne_getElem! _ _ 3 0 (by grind) (by grind) (by omega)]
      simp only [a7_post, Array.set_of_ne_getElem! _ _ 3 1 (by grind) (by grind) (by omega)]
      simp only [a6_post, Array.set_of_ne_getElem! _ _ 3 0 (by grind) (by grind) (by omega)]
    have ha''_4 : a''[4]!.val = a'[4]!.val := by
      simp only [a''_post, Array.set_of_ne_getElem! _ _ 4 0 (by grind) (by grind) (by omega)]
    ...
    · -- Field51_as_Nat res ≡ (Field51_as_Nat a)^(2^k.val) [MOD p]
      have hsq : Field51_as_Nat a'' ≡ (Field51_as_Nat a)^2 [MOD p] :=
        field51_sq_modeq a' a'' carry c0.val c1.val c2.val c3.val c4.val
          c1'.val c2'.val c3'.val c4'.val
          ha'_0 ha'_1 ha'_2 ha'_3 ha'_4
          ha''_0 ha''_1 ha''_2 ha''_3 ha''_4
          hc1'_eq hc2'_eq hc3'_eq hc4'_eq hcarry_val a_pow_two
...
end curve25519_dalek.backend.serial.u64.field.FieldElement51.v2
