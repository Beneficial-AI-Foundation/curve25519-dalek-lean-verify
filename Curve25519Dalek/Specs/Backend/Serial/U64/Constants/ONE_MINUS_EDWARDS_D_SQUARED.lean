...
theorem ONE_MINUS_EDWARDS_D_SQUARED_spec :
    ok ONE_MINUS_EDWARDS_D_SQUARED ⦃ _ => Field51_as_Nat ONE_MINUS_EDWARDS_D_SQUARED = (1 + p - (d^2 % p)) % p ⦄ := by
  have h : Field51_as_Nat ONE_MINUS_EDWARDS_D_SQUARED = (1 + p - (d^2 % p)) % p := by
