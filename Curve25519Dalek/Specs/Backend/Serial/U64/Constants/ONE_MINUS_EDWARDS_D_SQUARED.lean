...
natural language description:
    ...
    • constants.ONE_MINUS_EDWARDS_D_SQUARED is a constant representing 1 - d² (mod p)
      whereby d is a parameter in the defining curve equation ax^2 + y^2 = 1 + dx^2y^2.
    • The field element constants.ONE_MINUS_EDWARDS_D_SQUARED is represented as five u64 limbs
      (51-bit limbs)
    • Note that the original Rust comment states
      "One minus edwards `d` value squared, equal to `(1 - (-121665/121666) mod p) pow 2`",
      however, there seems to be a typo/error in this comment, as it appears that the constant
      (as defined) is congruent to 1 - d^2 and not to (1 - d)^2.

natural language specs:

    • Field51_as_Nat(constants.ONE_MINUS_EDWARDS_D_SQUARED) = (1 - d²) (mod p) where d is the
      mathematical representation of the Edwards curve parameter as a natural number.
-/
