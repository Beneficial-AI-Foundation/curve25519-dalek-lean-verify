...
- The function always succeeds (no panic) for valid 64-byte input
...
- If the input bytes are uniformly distributed, the output point is uniformly
  ...
  require more extensive probability theory formalizations beyond the current scope of this project)
-/
