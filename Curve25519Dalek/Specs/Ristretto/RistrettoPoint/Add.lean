...
the concrete representative of the output Ristretto point on the curve. Since the addition on
RistrettoPoints is mathematically well-defined (i.e., it does not depend on the choice of
representatives), the condition

result.toPoint = self.toPoint + other.toPoint

thus indeed implies that the output RistrettoPoint (seen as an equivalence class) is the
mathematically correct sum of the input RistrettoPoints (seen as equivalence classes), even though
we are only working at the level of (fairly arbitrary) representatives.

The fact that the addition of RistrettoPoints is indeed well-defined and does not depend on the
chosen representatives follows from standard results in abstract algebra: in any set of left cosets
G/N, the product

(aN)(bN)=(ab)N

constitutes a well-defined operation that does not depend on the chosen representatives a, b
iff N is a normal subgroup;
