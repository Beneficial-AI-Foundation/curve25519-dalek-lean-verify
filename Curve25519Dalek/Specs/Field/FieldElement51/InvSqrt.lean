...
theorem invsqrt_spec
    ...
    invsqrt v ⦃ res =>
      ...
      -- Case 2: v ≢ 0 and ∃ x, x^2 * v ≡ 1 (mod p) → c.val = 1 and r^2 * v ≡ 1 (mod p)
      (v_nat ≠ 0 ∧ (∃ x : Nat, (x ^ 2 * v_nat) % p = 1) →
        res.fst.val = 1#u8 ∧ (r_nat ^ 2 * v_nat) % p = 1) ∧
      -- Case 3: v ≢ 0 and ¬∃ x, x^2 * v ≡ 1 (mod p) → c.val = 0 and r^2 * v ≡ i (mod p)
      (v_nat ≠ 0 ∧ ¬(∃ x : Nat, (x ^ 2 * v_nat) % p = 1) →
  ...
  have h_one : Field51_as_Nat
      (Array.make 5#usize [1#u64, 0#u64, 0#u64, 0#u64, 0#u64]) % p = 1 := by decide
