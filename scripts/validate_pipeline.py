...
def aggregate(functions: list[dict]) -> dict:
    ...
    return {
        ...
        "ignored": sum(1 for f in functions if f.get("is_ignored")),
    }


COMPARE_FIELDS = [
    "is_hidden", "is_ignored", "is_extraction_artifact",
    "specified", "verified", "externally_verified",
]
...
def compare(old_fns: list[dict], new_fns: list[dict]) -> str:
    ...
    for metric in ["total", "visible", "verified", "specified",
                    "externally_verified", "fully_verified", "ignored"]:
        o = old_agg[metric]
        n = new_agg[metric]
        delta = n - o
