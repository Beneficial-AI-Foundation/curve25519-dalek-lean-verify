...
#   ./scripts/bench/profile-report.sh <profile.json> -o report.txt  # save to file

set -e

if [ -z "$1" ] || [ ! -f "$1" ]; then
    echo "Usage: $0 <profile.json> [--top N] [--by METRIC] [-o FILE]"
    echo ""
    echo "Options:"
    echo "  --top N       Show top N files (default: 20)"
    echo "  --by METRIC   Sort by metric (default: simp_s)"
    ...
    echo "  $0 benchmarks/profile-20260121/profile.json -o reports/baseline.txt"
    exit 1
fi
