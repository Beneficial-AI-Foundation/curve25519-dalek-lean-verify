...
theorem my_fun_spec_fix (input : Array U64 5#usize) :
  ...
  -- Wrap it in `Hold` (opaque to simp) before reducing the `to_slice` bind.
  have h_mod' : Hold (U8x32_as_Nat bytes ≡ Field51_as_Nat input [MOD p]) := h_mod
  clear h_mod
  -- Reduce the pure `to_slice` bind manually (restricted to goal only, avoiding hypotheses)
  ...
  -- SliceU8_as_Nat (to_slice bytes) = U8x32_as_Nat bytes  (to_slice preserves .val)
  exact h_mod'
